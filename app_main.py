...
######################################################    
    ...
    # with st.container():
        ...
        if "photo_ai_verified" not in st.session_state:
            st.session_state.photo_ai_verified = False

        st.info("📷 **Webcam Instructions:** Click 'Take Photo' below and allow camera access.")

        camera_photo = st.camera_input(
            "Take a live photo* (Mandatory)",
            help="Position your face clearly in the frame",
        ...
        if camera_photo:
            ...
            if not ai_helper.has_face(camera_photo):
                st.error("❌ No face detected! Please take a clear photo of your face.")
                st.session_state.photo_ai_verified = st.session_state.camera_photo_kyc = False
            elif ai_helper.has_face(camera_photo) > 1:
                st.error("❌ Multiple face detected! Please take single person photo.")
                st.session_state.photo_ai_verified = st.session_state.camera_photo_kyc = False
            else:
                # If face exists, proceed to AI liveness check
                with st.spinner("🔍 Verifying photo authenticity using AI..."):
                    base64_img = ai_helper.image_file_to_base64(camera_photo)
                    ai_result = ai_helper.openai_photo_liveness_check(base64_img)

                if ai_result["is_live"]:
                    st.success("✅ Photo verified as live human (AI check passed)")
                    st.session_state.photo_ai_verified = st.session_state.camera_photo_kyc = True
                else:
                    st.error(f"❌ Photo rejected: {ai_result['reason']}")
                    st.session_state.photo_ai_verified = st.session_state.camera_photo_kyc = False
...
############################################################## 
    ...
    # --------------------------------------------------
    # 📹 video Identification start 
    # --------------------------------------------------

    with st.container():
        st.markdown("### 📹 Video Identification – Live Video Verification")

        st.info(
            "✔ Look straight into the camera for 10 seconds\n"
        ...
        if not webrtc_ctx.state.playing:
            status_placeholder.info("📷 Initializing camera… Please allow camera access.")
        else:
        ...
        # ---------------- Verification Button ----------------
        verify_disabled = not st.session_state.camera_ready

        if st.button(
            "▶ Start 10-Second Video Verification",
            ...
            fail_frames = 0
            max_allowed_fails = 40
            multiple_faces = False

            while time.time() - start_time < duration:
                remaining = int(duration - (time.time() - start_time))
                countdown_placeholder.metric("⏳ Verifying", f"{remaining}s")

                if face_counts:
                    avg_faces = round(sum(face_counts) / len(face_counts))
                ...
                if fail_frames > max_allowed_fails:
                    break
            ...
            if multiple_faces:
                result_placeholder.error("❌ Failed: Multiple faces detected.")
                st.session_state.liveness_verified = liveness_verified = False

            elif fail_frames > max_allowed_fails:
                result_placeholder.error("❌ Failed: Face not detected consistently.")
                st.session_state.liveness_verified = liveness_verified = False

            else:
                result_placeholder.success("✅ Video Liveness Verified Successfully")
                st.session_state.liveness_verified = liveness_verified = True

        st.markdown("---")


    #--------------------------------------------------
    # 📹 video Identification start end 
    # --------------------------------------------------

        openai_liveness_check = False
        with st.form("kyc_portal_form", clear_on_submit=False):
            ...
            submitted = st.form_submit_button("Submit KYC Application", use_container_width=True, type="primary", disabled=not st.session_state.liveness_verified)
            if not st.session_state.liveness_verified:
                st.warning("🔒 Please complete Video Liveness Verification to enable submission.")
            else:
                st.success("✅ Video Liveness Verified. You may submit your KYC.")
            ...
            if submitted:
                ...
                # liveness_verified = st.session_state.liveness_verified
                ...
                #     missing_fields.append("Video (Webcam - Mandatory)")
                if missing_fields:
